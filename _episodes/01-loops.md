...
In this lesson we will build on top of what we learned on the previous lesson **An Introduction to Linux with Command Line**. 
We will continue working with Nelle's pipeline and expand it to make use of Bash scripts to automatize the production of results using many of the Bash commands already learned.
...
> ## Variables in Loops
>
> This exercise refers to the `data-shell-scripting/molecules` directory.
...
Let's continue with our example in the `data-shell-scripting/creatures` directory.
...
We would like to modify each of the files in `data-shell-scripting/creatures`, but also save a version
